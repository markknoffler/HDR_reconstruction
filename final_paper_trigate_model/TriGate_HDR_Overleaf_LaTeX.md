# TriGate-HDR — Overleaf / LaTeX Source

This file contains the **complete, literal LaTeX source** for the TriGate-HDR CVPR paper. It is delivered as a `.md` file for portability, but every code block below is meant to be copied directly into Overleaf (or compiled locally with `pdflatex` + `bibtex`).

## How to use this in Overleaf

1. Create a new **Blank Project** in Overleaf.
2. Replace the contents of `main.tex` with the contents of the **`main.tex`** code block below.
3. Create a new file named **`references.bib`** and paste the contents of the **`references.bib`** code block below.
4. Create a folder named **`figures/`** and upload `architecture_diagram.png` from the assets ZIP into it (the file is already named correctly to match the `\includegraphics` call).
5. If your venue uses the official CVPR LaTeX template (`cvpr.sty`, `eso-pic`, etc.), upload those style files into the project root — the code below uses the standard `\documentclass[10pt,twocolumn,letterpaper]{article}` skeleton with CVPR-style macros defined inline so it compiles standalone even without the official template package. Swap in `\documentclass{cvpr}` if you have the official CVPR 2026 kit.
6. Compile with **pdfLaTeX → BibTeX → pdfLaTeX → pdfLaTeX** (standard four-pass cycle for references and citations to resolve).
...
The `\xmark` and `\cmark` symbols used in Table 2 require the `pifont` package, which is already included in the preamble above along with the macro definitions:
...
If you have access to the official CVPR 2026 author kit (`cvpr.sty`), swap the bare `article` class and geometry/hyperref preamble lines for `\documentclass[review]{cvpr}` and remove the manual `\usepackage{geometry}` call, since the official class defines the two-column layout, headers, and anonymization block automatically. The figure path `figures/architecture_diagram.png` matches the filename included in the assets ZIP — no renaming necessary after upload.
