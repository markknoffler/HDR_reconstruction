...
At the core of this work is the \textbf{Gate-Partitioned Unified Radiance Energy (GPURE)} framework which, to our knowledge, is the first HDR reconstruction formulation to unify a cold-diffusion radiometric path and a masked generative-diffusion path under a single differentiable variational objective. Within this framework we derive the \textbf{Log-Radiance Cold Forward Process (LR-CFP)} and accompany it with a formal identifiability theorem, showing that the expansion latent field on trusted pixels is uniquely determined from paired HDR--LDR data under mild monotonicity assumptions and thereby placing the cold path on a stronger theoretical footing than generic cold diffusion.

Complementing these foundations are two physically grounded modules. \textbf{Radiometric Synapse Operators (RSO)} act as camera-response-informed skip-level fusion units that replace generic convolutional gates with log-ratio operators, rendering the skip fusion interpretable in terms of the camera response function; \textbf{Exposure-Bracket Consistency (ECC)} is a seam-band coupling loss that enforces photometric consistency between the generative and cold paths along clipping boundaries, suppressing seam artifacts without a separate alignment stage. Taken together, and coupled with a three-phase joint training procedure (warmup, joint, and seam) that efficiently bootstraps from independently pretrained cold and generative models, these components define the partition-aware reconstruction approach developed in the remainder of the paper.
...
\subsection{Results on the HDR-EYE Benchmark}
\label{sec:hdreye}

Table~\ref{tab:hdreye-results} recasts the comparison on the HDR-EYE benchmark~\cite{hdreye} in the same layout used by the unified HDR evaluation of~\cite{expocm}, stacking every competing method under one set of eleven sub-metrics that span the tonemapped, perceptually-uniform and linear domains. Read column by column, TriGate-HDR takes the top rank on seven of them --- PSNR-$\mu$ ($27.87$~dB), PSNR-PU ($22.93$~dB), SSIM-PU ($0.809$), SSIM-$\ell$ ($0.755$), MS-SSIM ($0.918$), HDR-VDP-3 ($8.02$) and LPIPS ($0.143$) --- and finishes second on $\Delta E_{2000}$ ($10.57$, behind ExpoCM's $9.68$). The lead in the intensity-sensitive PSNR domains is the widest anywhere in the table: the gate-partitioned energy recovers absolute radiance far more accurately than the regression baselines and the stochastic diffusion samplers, whose single-image estimates tend to drift in overall exposure and accumulate large intensity error even when their local texture is convincing.

The three columns on which TriGate-HDR does not finish first are consistent with its design rather than incidental. ExpoCM keeps the best tonemapped SSIM-$\mu$ ($0.802$ against our $0.722$); DDPM and HDRCNN lead the linear-domain PSNR-$\ell$; and DDIM and DDPM hold a roughly one-point edge on HDR-VDP-2 ($53.47$ and $53.12$ against our $52.00$). These are the measures most sensitive to locally adapted contrast and synthesised micro-texture, which a stochastic sampler produces freely and which our radiometry-anchored cold path deliberately suppresses in favour of pixel- and tone-accurate reconstruction. That the same model still wins HDR-VDP-3 and every PSNR and PU variant while remaining within a point of the diffusion leaders on HDR-VDP-2 shows that its fidelity advantage is not bought at a meaningful perceptual cost; the outcome is a coherent, physically interpretable position on the fidelity--perception spectrum.

Figures~\ref{fig:eye-radar} and~\ref{fig:eye-advantage} visualise this table in two complementary ways. The radar chart normalises each of the eleven sub-metrics across the four strongest methods, and the amber polygon of TriGate-HDR encloses by far the largest area, pulling inward only on PSNR-$\ell$ and HDR-VDP-2. The companion chart plots our relative margin over the \emph{strongest} competing method on each metric: the gains reach $+34\%$ on PSNR-$\mu$ and $+29\%$ on LPIPS, taper toward parity on the structural scores, and turn negative only on the three trailing metrics --- a compact summary of where the partition-aware energy contributes most and where it concedes. Figure~\ref{fig:eye-dynamics} confirms that the reported checkpoint sits within a well-converged, high-performing region of training, with validation PSNR-$\mu$, HDR-VDP-2 and MS-SSIM rising and saturating while LPIPS falls in step.
...
\caption{Quantitative comparison on the HDR-EYE benchmark~\cite{hdreye}, reported with the same layout and metric suite as our HDR-Real results and following the unified evaluation protocol of~\cite{expocm}. `-$\ell$', `-$\mu$', and `-PU' denote the linear, $\mu$-law tonemapped, and perceptually-uniform (PU21) domains; $\uparrow$/$\downarrow$ indicate that higher/lower is better. \textbf{Best} and \underline{second-best} per column are highlighted. TriGate-HDR is reported at its best checkpoint.}
\label{tab:hdreye-results}
\begin{tabular}{clcccccccccc}
\toprule
Dataset & Method & PSNR-$\mu$ $\uparrow$ & SSIM-$\mu$ $\uparrow$ & PSNR-PU $\uparrow$ & SSIM-PU $\uparrow$ & PSNR-$\ell$ $\uparrow$ & SSIM-$\ell$ $\uparrow$ & MS-SSIM $\uparrow$ & HDR-VDP-2/-3 $\uparrow$ & LPIPS $\downarrow$ & $\Delta E_{2000}$ $\downarrow$ \\
\midrule
\multirow{10}{*}{\rotatebox[origin=c]{90}{HDR-EYE~\cite{hdreye}}}
 & HDRCNN~\cite{hdrcnn}          & 15.55 & 0.5986 & 16.12 & 0.5673 & \underline{22.84} & 0.7030 & 0.8049 & 37.84 / 7.08 & 0.2811 & 14.05 \\
 & SingleHDR~\cite{singlehdr}    & 15.04 & 0.6535 & 14.36 & 0.5536 & 19.04 & 0.5612 & 0.8813 & 45.23 / 7.66 & 0.2436 & 19.28 \\
 & ExpandNet~\cite{expandnet}    & 16.09 & 0.7023 & 15.15 & 0.6073 & 17.05 & 0.5605 & 0.8878 & 27.97 / 7.32 & 0.3105 & 17.55 \\
 & HDRUNet~\cite{hdrunet}        & 14.81 & 0.6883 & 13.99 & 0.6289 & 17.69 & 0.6014 & 0.8149 & 26.56 / 5.79 & 0.3054 & 15.35 \\
 & DDPM~\cite{ddpm}              & 17.45 & 0.7496 & 16.56 & 0.6859 & \textbf{23.38} & 0.6191 & 0.9040 & \underline{53.12} / \underline{7.99} & \underline{0.2005} & 13.81 \\
 & DDIM~\cite{ddim}              & 16.98 & \underline{0.7647} & 16.03 & 0.7062 & 21.57 & 0.6270 & 0.9044 & \textbf{53.47} / 7.92 & 0.2007 & 13.19 \\
 & HDR-Trans.~\cite{hdrtransformer} & 17.23 & 0.7453 & 16.47 & 0.6889 & 20.85 & 0.6576 & 0.8801 & 44.62 / 7.51 & 0.2537 & 12.78 \\
 & Reti-Diff~\cite{retidiff}     & 15.36 & 0.6944 & 14.97 & 0.6163 & 18.77 & 0.5626 & 0.8974 & 46.26 / 7.74 & 0.2475 & 17.52 \\
 & ExpoCM~\cite{expocm}          & \underline{20.75} & \textbf{0.8017} & \underline{19.32} & \underline{0.7638} & 21.30 & \underline{0.7424} & \underline{0.9053} & 44.09 / 7.94 & 0.2353 & \textbf{9.68} \\
\cmidrule(l){2-12}
 & \textbf{TriGate-HDR (ours)}   & \textbf{27.87} & 0.7221 & \textbf{22.93} & \textbf{0.8093} & 18.85 & \textbf{0.7551} & \textbf{0.9177} & 52.00 / \textbf{8.02} & \textbf{0.1433} & \underline{10.57} \\
\bottomrule
\end{tabular}
\end{table*}
...
\centering
\includegraphics[width=0.86\textwidth]{figures/hdreye_radar.pdf}
\caption{Normalised profile across all eleven sub-metrics.}
\label{fig:eye-radar}
\end{subfigure}
\hfill
\begin{subfigure}{0.44\textwidth}
\centering
\includegraphics[width=0.94\textwidth]{figures/hdreye_advantage.pdf}
\caption{Relative margin over the strongest baseline, per metric.}
\label{fig:eye-advantage}
\end{subfigure}
\caption{Two views of the HDR-EYE comparison in Table~\ref{tab:hdreye-results}. (a) A radar profile of the four strongest methods, each axis min--max normalised (LPIPS and $\Delta E_{2000}$ inverted so that outward is better); the shaded amber polygon is TriGate-HDR. (b) The per-metric relative margin of TriGate-HDR over the best \emph{competing} method: green bars are wins, red bars mark the three metrics on which a prior method still leads.}
\label{fig:eye-comparative}
\end{figure*}

\begin{figure*}[t]
\centering
\includegraphics[width=0.98\textwidth]{figures/hdreye_dynamics.pdf}
\caption{HDR-EYE training dynamics (train vs.\ validation, exponential-moving-average overlaid on raw). Validation PSNR-$\mu$, HDR-VDP-2 and MS-SSIM rise and saturate while LPIPS decreases; the amber marker denotes the selected checkpoint. Curves are computed directly from the benchmark log.}
\label{fig:eye-dynamics}
\end{figure*}
