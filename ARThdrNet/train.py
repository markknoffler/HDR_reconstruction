...
#                gt_tonemapped = mu_tonemap(hdr_gt[batch_ind:batch_ind+1])
                ...
                psnr_val = 10 * np.log10(1.0 / (mse.item() + 1e-10))
...
def train():
    ...
    model = ArtHDRNet(in_channels=3, base_channels=64, num_iterations=4).to(DEVICE)
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        for batch_idx, (ldr_gt, ldr_his, hdr_gt, _) in enumerate(pbar):
            ...
            with torch.no_grad():
                hdr_gt_tm = criterion.mu_law_compression(hdr_gt)
                hdr_pred_tm = criterion.mu_law_compression(hdr_outputs[-1])
                l1_val = criterion.l1_loss(hdr_pred_tm, hdr_gt_tm).item()
                per_val = criterion.perceptual_loss(hdr_pred_tm, hdr_gt_tm).item()
                epoch_components['l1'] += l1_val
                epoch_components['perceptual'] += per_val
        ...
        print(f"  L1: {epoch_components['l1']:.4f}, Perceptual: {epoch_components['perceptual']:.4f}")
        ...
        csv_writer.writerow([
            ...
            epoch_components['l1'], epoch_components['perceptual'],
