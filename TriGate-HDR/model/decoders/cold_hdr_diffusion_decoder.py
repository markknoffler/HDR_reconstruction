...
class ColdHDRDiffusion(nn.Module):
    def __init__(self, model=None, timesteps=100):
        super().__init__()
        self.model = model if model is not None else HDR_UNet(base_ch=64)
    ...
    def forward(self, x_hdr):
        ...
        x_pred = self.model(x_clipped, t)
        hdr_loss = F.l1_loss(x_hdr, x_pred)
        ldr_loss = F.l1_loss(x_clipped, self.move_toward_ldr(x_pred, t))
        return x_pred, {"hdr_loss": hdr_loss, "ldr_loss": ldr_loss, "loss": hdr_loss + ldr_loss}
    ...
    def restore_hdr(self, x_ldr):
        ...
        for t in reversed(range(self.timesteps)):
            t_batch = torch.full((batch_size,), t, device=x_ldr.device, dtype=torch.long)
            x0_guess = self.model(img, t_batch)
            if t > 0:
                img = img - self.move_toward_ldr(x0_guess, t_batch) + self.move_toward_ldr(x0_guess, t_batch - 1)
            else:
                img = x0_guess
        return img
