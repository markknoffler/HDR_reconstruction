...
class material_encoder(nn.Module):
    def __init__(self, in_channels=3, base_channels=64, num_blocks=4, use_grad=True, use_hist=True, use_gram=True):
    ...
    def forward(self, x):
        ...
        if self.use_hist:
            local_means = self.avg_pool(x)  # (B, 3, H, W)
            handcrafted.append(local_means)

        if self.use_gram:
            mu = self.avg_pool(x)  # (B, 3, H, W)
            rr = x[:, 0:1, :, :] * x[:, 0:1, :, :]
            gg = x[:, 1:2, :, :] * x[:, 1:2, :, :]
            bb = x[:, 2:3, :, :] * x[:, 2:3, :, :]
            rg = x[:, 0:1, :, :] * x[:, 1:2, :, :]
            ...
            gram_feat = torch.cat([cov_rr, cov_gg, cov_bb, cov_rg, cov_rb, cov_gb], dim=1)
            handcrafted.append(gram_feat)

        if handcrafted:
            material_feat = torch.cat([cnn_feat] + handcrafted, dim=1)
        else:
            material_feat = cnn_feat

        return material_feat
